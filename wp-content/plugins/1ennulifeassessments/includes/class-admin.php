...
class ENNU_Admin {
    ...
    private function display_assessment_fields($user_id, $assessment_type, $assessment_label) {
        ...
        $meta_prefix = 'ennu_' . $assessment_type . '_';
        $all_meta = get_user_meta($user_id);
        
        $assessment_fields = array();
        foreach ($all_meta as $key => $value) {
            if (strpos($key, $meta_prefix) === 0 && !strpos($key, '_date') && !strpos($key, '_label')) {
        ...
        if (!empty($assessment_fields)) {
            ...
            foreach ($assessment_fields as $field_key => $field_data) {
                ...
                echo '<tr>';
                echo '<th><label>' . esc_html($label) . $date . '</label></th>';
                echo '<td>';
                echo '<input type="text" name="' . esc_attr($meta_prefix . $field_key) . '" value="' . esc_attr($field_data['value']) . '" class="regular-text" readonly />';
                echo '<p class="description">Assessment response (read-only)</p>';
                echo '</td>';
                echo '</tr>';
            }
            ...
            $last_updated = get_user_meta($user_id, $meta_prefix . 'last_updated', true);
            ...
            if ($completion_status) {
                ...
                if ($last_updated) {
                    echo ' - Last updated: ' . date('M j, Y g:i A', strtotime($last_updated));
                }
                echo '</td>';
                echo '</tr>';
            }
