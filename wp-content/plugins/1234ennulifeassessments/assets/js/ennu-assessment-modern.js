...
// Show error message - UPDATED to handle HTML
        showError: function(message) {
            // Use .html() instead of .text() to render the login link
            var $error = $('<div class="ennu-message error"></div>').html(message);
            $('.ennu-question-step.active').prepend($error);
        
            // Remove error after 8 seconds to give user time to click
            setTimeout(() => {
                $error.fadeOut(() => $error.remove());
            }, 8000);
        
            // Reset submit button
            var $submitBtn = $('.ennu-submit-btn, .submit-assessment-btn');
            $submitBtn.prop('disabled', false);
        
            // Restore original button text
            var originalText = $submitBtn.data('original-text') || 'View My Assessment Results';
